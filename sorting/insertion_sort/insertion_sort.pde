...
void drawArrayBars() {
  ...
  for (int k = 0; k < n; k++) {
    ...
    if (sorted) {
      ...
      fill(255, 165, 0); // Orange for current key being inserted
      currentKey = arr[k]; // Store the current key for display
    } else if (k == j + 1 && j >= 0) {
      fill(255, 0, 0); // Red for position where key might be inserted
    } else if (k <= j && k >= 0) {
      fill(70, 70, 220); // Blue for elements being shifted
    } else {
...
void updateSorting() {
  if (!paused && !sorted) {
    if (isSwapping) {
      // Handle animation of swapping/shifting
      animationProgress += 0.1;
      if (animationProgress >= 1) {
        // Complete the swap/shift
        int temp = arr[j + 1];
        arr[j + 1] = arr[j];
        arr[j] = temp;
        isSwapping = false;
      ...
      if (i < n) {
        if (j >= 0) {
          ...
          if (arr[j] > arr[j + 1]) {
            ...
            // Found correct position, move to next element
            i++;
            j = i - 1;
            steps++;
          }
        } else {
          // Reached beginning of array, move to next element
          i++;
          j = i - 1;
          steps++;
        }
...
void drawInfoPanel() {
  ...
  text("Insertion Sort Visualizer", panelX + panelWidth/2, panelY + 15);

  // Draw algorithm information
  textSize(14);
  textAlign(LEFT, TOP);
  fill(0);
  String info = "Insertion Sort builds the sorted\narray one item at a time by\ncomparing each new element with\nthe already sorted elements and\ninserting it at the correct position.";
  text(info, panelX + 15, panelY + 45);

  // Draw complexity information
  text("Time Complexity: O(n²)", panelX + 15, panelY + 130);
  text("Space Complexity: O(1)", panelX + 15, panelY + 150);

  // Draw current metrics
  textAlign(LEFT, TOP);
  text("Steps: " + steps, panelX + 15, panelY + 175);
  text("Comparisons: " + comparisons, panelX + 150, panelY + 175);
  text("Swaps: " + swaps, panelX + 15, panelY + 195);
  ...
  if (sorted) {
    ...
    action = "Shifting element " + arr[j] + " to make room for " + currentKey;
    fill(255, 140, 0);
  } else {
    action = "Finding correct position for " + (i < n ? arr[i] : "");
    fill(0, 0, 150);
  }
