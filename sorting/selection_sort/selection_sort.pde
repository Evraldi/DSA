...
void drawArrayBars() {
  ...
  for (int k = 0; k < n; k++) {
    ...
    if (sorted) {
      ...
      fill(255, 165, 0); // Orange for current position being sorted
    } else if (k == minIndex) {
      fill(255, 0, 0); // Red for current minimum element
    } else if (k == j) {
      fill(255, 0, 255); // Purple for element being compared
    } else {
  ...
  if (!paused && !sorted) {
    if (isSwapping) {
      ...
      if (j < n) {
        ...
        // If minIndex is not i, we need to swap
        if (minIndex != i) {
          isSwapping = true;
          animationProgress = 0;
        } else {
...
void drawInfoPanel() {
  ...
  text("Selection Sort Visualizer", panelX + panelWidth/2, panelY + 15);

  // Draw algorithm information
  textSize(14);
  textAlign(LEFT, TOP);
  fill(0);
  String info = "Selection Sort finds the minimum\nelement from the unsorted part\nand puts it at the beginning.\nThis process continues until\nall elements are sorted.";
  ...
  if (sorted) {
    ...
    action = "Swapping elements " + arr[i] + " and " + arr[minIndex];
    fill(255, 140, 0);
  } else {
    action = "Finding minimum in unsorted portion";
    fill(0, 0, 150);
  }
